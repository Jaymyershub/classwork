...
//                            datatype  variable-name    = initial-value  ;
    ...
    /***************************************************************************************************
        C#: MethodNames are Pascal case
                Opening brace for the method is on the next line from the method signature
                (same is true for any statement that has a block - if, while, switch, etc)

        Java methodNames are camelCase
    ...
    public int GetIntValue() {
      // Return integer value of CardValue enum in a PlayingCard
       // return (int) value;  // C# - cast the enum to an int to get its integer value
        return value.ordinal(); // Java: .oridinal() method return the enums int value
    }
    ...
    private void setColor(CardSuit suit) {  // Set the color based on the suit of the object
        ...
        switch (suit)
        {
            ...
            case CardSuit.Heart:
                ...
                break;      // break is required so we don't fall through to the next case - we exit the switch
            default:
                this.color = DEFAULTCOLOR;  // break is optional in JAVA
                break;
        }
    }
